...
- **[spawn-worktree-agent](./skills/spawn-worktree-agent/SKILL.md)** — Delegates a problem or investigation to an autonomous coding-agent worker (Claude by default; any Herdr `--kind`) in an isolated Herdr worktree — including nested (a worker spawning another worker) and cross-repo (a worker for a different repo than the session's). Uses `herdr-worktree` to create the worktree, then opens two tabs — `git` (lazygit) and one named after the agent — launching the worker with the task delivered as its startup argument (not typed into the prompt box after the fact) and verifying the worker actually begins before reporting success. Runs inside Herdr (>= 0.7.5) only.
