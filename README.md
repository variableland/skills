...
- **[spawn-worktree-agent](./skills/spawn-worktree-agent/SKILL.md)** — Delegates a problem or investigation to an autonomous coding-agent worker (Claude by default; any Herdr `--kind`) in an isolated Herdr worktree. Uses `herdr-worktree` to create the worktree, then opens two tabs — `git` (lazygit) and one named after the agent (the worker, autonomous) — submits a specialized prompt, and reports. Runs inside Herdr (>= 0.7.5) only.
